...
export default function Escalations() {
    const mockLogs = [
        { id: 'ES-9921', risk: 'High', status: 'Pending Review', time: '2 mins ago', hash: '8f7d9a...2b1c' },
        { id: 'ES-9920', risk: 'Medium', status: 'Routed to Counselor', time: '14 mins ago', hash: '4a2c1f...9e3d' },
        { id: 'ES-9919', risk: 'High', status: 'Intervention Active', time: '1 hour ago', hash: '1d8e5b...7f6a' },
        { id: 'ES-9918', risk: 'Medium', status: 'Resolved', time: '3 hours ago', hash: '5c3a2e...1b8f' },
    ];
    ...
    return (
        <div className="space-y-6 animate-[fade-in_0.3s_ease-out]">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-sora font-semibold text-white">Escalation Logs</h1>
                    <p className="text-sm font-mono text-cool-mist/60 mt-1">Real-time crisis ping routing and triage.</p>
                </div>
                ...
                <table className="w-full text-left font-mono text-sm text-cool-mist">
                    <thead className="bg-[#0A0A0C] border-b border-white/5 text-[10px] uppercase tracking-wider text-cool-mist/50">
                        <tr>
                            ...
                            <th className="px-6 py-4">Student Node Hash</th>
                    ...
                    <tbody className="divide-y divide-white/5">
                        {mockLogs.map((log, i) => (
                            <tr key={i} className="hover:bg-white/[0.02] transition-colors cursor-pointer group">
                                ...
                                <td className="px-6 py-4 font-mono text-xs opacity-50">{log.hash}</td>
                                ...
                                <td className="px-6 py-4 flex items-center gap-2 text-xs">
                                    ...
                                    {(log.status === 'Intervention Active' || log.status.includes('Routed')) && <Clock className="w-3 h-3 text-electric-lavender" />}
                                    {log.status}
                                </td>
